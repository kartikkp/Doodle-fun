...
- Final current runtime `9a819bdfd018e8e0`, HTML SHA-256 `7de741f6ab0d4e3144af2629ffa3b16fb6e519410362d4d8cfff8910d293ac9f`. Source, standalone and native bundle are synchronized. Application/test commit `b13a06d` includes the audio recovery fix and native capture/control fixes.
...
Current syntax checks and 79/79 Node tests passed. Complete independent CI on earlier application/test commit `f5e101b`, runtime `ad3544bbd06a552e`, passed 628/628 Chromium/WebKit cases, including all 84 listening cases without skips, an unsigned iPhone Release build, and 282/282 native gameplay/bridge/layout cases. CI run 34782058175 is rechecking the current runtime; its final results are not yet recorded here.

Local iPhone 17 Pro/iOS 26.5 and iPhone SE 3/iOS 18.6 each passed all 270 retained age/mode gameplay cases and 11 bridge/gate cases on `ad3544bbd06a552e`. Local trusted touch/audio runs then suffered severe host stalls. A real native Listen tap left the AudioContext suspended with a pending resume and no advancing clock; the app kept answers locked and offered retry. A later host check confirmed the Mac was locked. Speakers are present. Unlock is required before diagnosing this as an app-specific playback defect or claiming native listening works.

The owner has been asked to unlock the Mac and reply when ready. No further local UI runs while locked. The final audio recovery fix addresses a separately confirmed lifecycle race; it does not prove the initial native playback issue is resolved. Four trusted native audio playthroughs are being added to CI for independent WKWebView validation.

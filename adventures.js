...
export function createAdventures(container,{getSettings,getTitle=()=>null,onBack=()=>{},onNotice=()=>{},onProgress=()=>{}}) {
  ...
  function render(){
    $('.adventure-title').textContent=getTitle() || ADVENTURE_TITLES[id];$('.adventure-round').textContent=`${profile.name} · round ${state.index+1}`;$('.adventure-hear').disabled=!getSettings().sound||!canSpeak();play.replaceChildren();container.dataset.adventure=id;
